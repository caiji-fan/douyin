...
func TestGetCommentBOS(t *testing.T) {
	var coms []po.Comment = []po.Comment{
		...
		{po.EntityModel{6, time.Now(), time.Now()}, 2, 456, "垃圾", '0'},
		{po.EntityModel{5, time.Now(), time.Now()}, 1, 666, "别说话", '0'},
		{po.EntityModel{8, time.Now(), time.Now()}, 1, 777, "尼玛", '0'},
		{po.EntityModel{4, time.Now(), time.Now()}, 1, 888, "发", '0'},
		{po.EntityModel{10, time.Now(), time.Now()}, 1, 999, "还是", '0'},
		{po.EntityModel{1, time.Now(), time.Now()}, 1, 000, "含税", '0'},
	}
...
func TestGetUserBOS(t *testing.T) {
	var users []po.User = []po.User{
		{po.EntityModel{9, time.Now(), time.Now()}, "张三", "siw", 1, 2},
		{po.EntityModel{7, time.Now(), time.Now()}, "李斯", "siw", 3, 4},
		{po.EntityModel{99, time.Now(), time.Now()}, "张静怡", "siw", 5, 6},
		{po.EntityModel{8, time.Now(), time.Now()}, "张诺", "siw", 7, 8},
...
func TestGetVideoBOS(t *testing.T) {
	var videos []po.Video = []po.Video{
		...
		{po.EntityModel{4, time.Now(), time.Now()}, "x3x.com", "s45l.cn", 666, 0, 2, "xx"},
		{po.EntityModel{5, time.Now(), time.Now()}, "x4x.com", "s7dl.cn", 0, 0, 1, "xx"},
		{po.EntityModel{6, time.Now(), time.Now()}, "x5x.com", "sd56l.cn", 0, 0, 4, "xx"},
	}
